...
#Fit the models
#Poisson model
poisson_model <- glm(glasses ~ eyesight + sex + sleep_wkdy,
											data = nlsy, family = poisson()
)

#Create the tables
poisson_model <- tbl_regression(
	poisson_model,
	exponentiate = TRUE,
	label = list(
		sex ~ "Sex",
...
#5
# using a logistic binomial regression model
tbl_uvregression(
	nlsy,
	y = glasses,
	include = c(
...
log_bino_model <- glm(glasses ~ eyesight_cat + sex_cat,
											data = nlsy, family = binomial(link = "log")
)

log_bino_model <- tbl_regression(
	log_bino_model,
	exponentiate = TRUE,
	label = list(
		sex_cat ~ "Sex",
...
#Creat and store logistic model
...
## Table comparing the Poisson and Logistic models
tbl_merge(list(logistic_model, log_bino_model),
					tab_spanner = c("**Logistic Model**", "**Logistic Binomial Model**")
)
